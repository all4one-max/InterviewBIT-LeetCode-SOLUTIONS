...
void solve() {
  int l, r; cin >> l >> r; vector<int> dp(4 * r, 0); int ans = 0;
  for (int g = r; g >= 2; g--) {
    int cnt = r / g - (l - 1) / g;
    int cur_ans = (cnt * cnt);
    if (g >= l) cur_ans -= (2 * (cnt - 1) + 1);
    for (int j = 2; j * g <= r; j++) cur_ans -= dp[j * g];
    ans += cur_ans; dp[g] = cur_ans;
    if (g >= l) dp[g] += (2 * (cnt - 1) + 1);
    // debug(g) debug(cur_ans) debug(cnt) debug(dp[g])
  }
  cout << ans << endl;
  return;
}

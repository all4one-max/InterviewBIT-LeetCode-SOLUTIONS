...
void sieve(int mx) { // O(n * log(log(n)))
  vector<int> v(mx, 1);
  v[0] = 0, v[1] = 0;
  for (int i = 2; i < mx; i++) {
    if (v[i]) {
...
void prime_factorisation(int num) { // O(log(n))
  while (num > 1) {
...
void solve() {
  int x, y;
  cin >> n >> x >> y;
  vector<int> a(n + 1, 0); fo(i, n) cin >> a[i];
  vector<int> nx(n + 1, INF); vector<int> ny(n + 1, INF); int behen_ka_loda = -1;
  int ans = 0;
  for (int i = n - 1; i >= 0; i--) {
    if (a[i] > x || a[i] < y) {
      behen_ka_loda = i;
      ...
      // cout << i << " " << nx[i] << " " << ny[i] << nl;
      if (nx[i] == INF || ny[i] == INF) continue;
      int mx = max(nx[i], ny[i]);
      if (behen_ka_loda != -1) ans += (behen_ka_loda - mx);
      else ans += (n - mx);
    }
  }
  cout << ans << nl;

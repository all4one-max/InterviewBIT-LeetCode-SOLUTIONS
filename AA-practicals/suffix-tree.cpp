...
public:
    SuffixTree(string& _s) {
        ...
        lastInternal = root;
        int i = 0;
        for (i = 0; i < n; i++) phase(i); // //for each phase, phase i
        i = n - 1;
    }
    int getIdx(char& c) {
        if (c == '$') return 26;
            ...
            while (j <= p && curr) {
                ...
                while (j <= p && k <= *(curr->en)) {
                    ...
                    if (s[j] == s[k]) {
                        ...
                        if (newNode->parent) newNode->parent->children[getIdx(s[newNode->st])] = newNode;
...
int main() {
    ...
    SuffixTree st(text);
    cout << (st.search("world") ? "FOUND PATTERN world" : "NOT FOUND \n");
}
